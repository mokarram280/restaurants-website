  ...
  return (
    <div className='w-[80%] mx-auto mt-32 relative'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-4 mt-10'>
            {ImageGallery.map((items,i)=>(
                <div key={i} className={`${i === 0 ? 'lg:col-span-2' : ''} ${ i === 0 ? 'lg:row-span-2' : ''}`}>
                    <img src={items.gallery} alt="" className='w-full h-full' />
                </div>
            ))}
        </div>
        <div className='my-12'>
        <Button aria-label='close popup' onPress={onOpen} className='w-[227px] h-[47px] font-medium rounded-full bg-white flex items-center justify-center gap-3 absolute bottom-10 left-10'><LuGrip size={20} />View all photos (7)</Button>
